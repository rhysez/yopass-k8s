...
## Getting started
You will find a set of shell scripts in `scripts/`. They deal with creating the cluster with `kind`, deploying container definitions to the cluster and load testing.

- You can create the cluster from the kind config found at the root of the project with `setup-cluster.sh`
- You can then deploy container definitions to the cluster with `deploy.sh`, this ensures that the cluster knows which containers it needs to run and where to pull the corresponding images from. 
...
The cluster contains **worker nodes**. These are essentially individual machines that run **pods** (containers/processes).
